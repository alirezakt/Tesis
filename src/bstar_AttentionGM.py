...
from .graph_memory_attention_module import GraphMemoryAttention
...
class STAR_GM(torch.nn.Module):
    def __init__(self, args, dropout_prob=0, nlayers = 2, nhead = 8, bilstm_hidden_size = 32):
        ...
        self.graph_memory_attention = GraphMemoryAttention(embed_dim=32, heads=4)
    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum > 0:
                ...
                attn_out = self.graph_memory_attention(
                    temporal_input_embedded[framenum].clone(),  # or .detach() if needed
                    GM[:framenum, node_index]
                )
                temporal_input_embedded = temporal_input_embedded.clone()
                temporal_input_embedded[framenum] = attn_out
