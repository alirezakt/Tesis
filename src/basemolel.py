...
class MakeMLP(nn.Module):
    ...
    def init_weights_nei(self,m):
        if type(m)==nn.Linear:
            ...
            nn.init.orthogonal(m.weight,gain=0.01)
            if False:
                nn.init.constant(m.bias,0)

    def init_weights_attr(self,m):
        if type(m)==nn.Linear:
            #nn.init.normal(m.weight,mean=0,std=0.05)
            nn.init.xavier_uniform(m.weight)
            try:
                nn.init.constant(m.bias,0)
            except:
...
class GCN(nn.Module):
    def __init__(self,args,r_embed_size,output_size):
        ...
        # Motion gate
        self.ngate = MakeMLP(self.args, 1, 'ngate', self.R+self.D + self.D, 64,
                                 self.D, 'sig', 0, ifbias=True)

        # Relative spatial embedding layer
        self.relativeLayer = MakeMLP(self.args, 1, 'rel', 2,
                                     16,
                                     self.R, 'relu', 0.1, ifbias=True, iflastac=True)
        # Message passing transform
        self.W_nei = MakeMLP(self.args, 1, 'nei', self.D, 64,
                         self.D, '', 0, ifbias=False,iflastac=False)

        tmp=self.R+self.D*2

        # Attention
        self.WAr = MakeMLP(self.args,1,'attR',tmp,self.D1,1, '', drop_ratio=0, ifbias=False)

        #not used
        self.WAr1 = MakeMLP(self.args,1,'attR',tmp,self.D1,32, '', drop_ratio=0, ifbias=False)
        self.WAr2 = MakeMLP(self.args, 1, 'attR2', self.D1, 32, 1, '', drop_ratio=0,ifbias=False)
