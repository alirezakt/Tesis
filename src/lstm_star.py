...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
        ...
        if self._qkv_same_embed_dim is False:
            self.q_proj_weight = nn.Parameter(torch.Tensor(embed_dim, embed_dim, dtype=torch.float32, device='cuda'))
            self.k_proj_weight = nn.Parameter(torch.Tensor(embed_dim, self.kdim, dtype=torch.float32, device='cuda'))
            self.v_proj_weight = nn.Parameter(torch.Tensor(embed_dim, self.vdim, dtype=torch.float32, device='cuda'))
    ...
    def forward(self, query, key, value, key_padding_mask=None,
                need_weights=True, attn_mask=None, attn_mask_weight = None):
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                v_proj_weight=self.v_proj_weight, attn_mask_weight = attn_mask_weight)
        else:
            return multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                ...
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask, attn_mask_weight=attn_mask_weight)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None, src_mask_weight = None):
        ...
        src2, attn = self.self_attn(src, src, src, attn_mask=src_mask,
                                    key_padding_mask=src_key_padding_mask, attn_mask_weight = src_mask_weight)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None, mask_weight = None):
        ...
        for i in range(self.num_layers):
            output, attn = self.layers[i](output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, src_mask_weight = mask_weight)
            atts.append(attn)
        if self.norm:
...
class TransformerModel(nn.Module):
    def __init__(self, args, ninp, nhead, nhid, nlayers, dropout=0.5):
    ...
    def forward(self, src, mask, mask_weight = None):
        n_mask = mask + torch.eye(mask.shape[0], mask.shape[0]).cuda() #کلیه اعداد قطر ماتریکس را برابر یک قرار میدهد یعنی هر عنصر با خودش ارتباط دارد
        n_mask = n_mask.float().masked_fill(n_mask == 0., float(-1e20)).masked_fill(n_mask == 1., float(0.0))  # mask 0 to 1e-20, mask 1 to 0.0

        #وزن تاثیر همسایه ها بین صفر و یک است. قطر برابر یک میشود. صفرها به منهای بینهایت تبدیل میشوند 
        n_mask_weight = None
        if mask_weight is not None: 
            n_mask_weight = mask_weight + torch.eye(mask_weight.shape[0], mask_weight.shape[0]).cuda() #کلیه اعداد قطر ماتریکس را برابر یک قرار میدهد یعنی هر عنصر با خودش ارتباط دارد
        #n_mask_weight = n_mask_weight.float().masked_fill(n_mask_weight == 0., float(-1e20))  # mask 0 to 1e-20    

        output = self.transformer_encoder(src, mask=n_mask, mask_weight=n_mask_weight)
        return output

class LSTMTransformerEncoder(nn.Module):
    def __init__(self, input_dim, lstm_hidden_dim, transformer_dim, nhead, num_layers, dropout=0.1):
        ...
        self.linear_proj = nn.Linear(lstm_hidden_dim, transformer_dim)
        ...
        self.output_proj = nn.Linear(transformer_dim, input_dim)  # back to 32

    def forward(self, x):
        # x: [seq_len, batch, input_dim]
        x = x.permute(1, 0, 2)  # to [batch, seq_len, input_dim] for LSTM
        lstm_out, _ = self.bilstm(x)  # [batch, seq_len, 2*hidden]
        proj = self.linear_proj(lstm_out)  # [batch, seq_len, transformer_dim]
        transformer_out = self.transformer_encoder(proj)  # [batch, seq_len, transformer_dim]
        out = self.output_proj(transformer_out)  # [batch, seq_len, input_dim]
        return out.permute(1, 0, 2)  # back to [seq_len, batch, input_dim]

class LSTM_STAR(torch.nn.Module):
    def __init__(self, args, dropout_prob=0, nlayers = 2, nhead = 8, bilstm_hidden_size = 32):
        super(LSTM_STAR, self).__init__()
        ...
        self.spatial_encoder_1 = TransformerModel(args, emsize, nhead, nhid, nlayers, dropout)
        self.spatial_encoder_2 = TransformerModel(args, emsize, nhead, nhid, nlayers, dropout)

        self.temporal_encoder_1 = LSTMTransformerEncoder(
            input_dim=32,
            lstm_hidden_dim=bilstm_hidden_size,
            transformer_dim=64,
            nhead=nhead,
            num_layers=nlayers,
            dropout=dropout_prob
        )
        self.temporal_encoder_2 = LSTMTransformerEncoder(
            input_dim=32,
            lstm_hidden_dim=bilstm_hidden_size,
            transformer_dim=64,
            nhead=nhead,
        ...
        self.relu = nn.ReLU()
        self.dropout_in = nn.Dropout(self.dropout_prob, inplace=False)
        self.dropout_in2 = nn.Dropout(self.dropout_prob, inplace=False)
        #self.dropout_out = nn.Dropout(0.1, inplace=False)

    def get_start_end(self, batch_num):
            """
    ...
    def update_batch_pednum(self, batch_pednum, ped_list):
        ...
        #calculate the number of pedestrians in each scene
    ...
    def forward(self, inputs, iftest=False):
        ...
        nei_weights = None
        #nodes_abs طول و عرض جغرافیایی
        num_Ped = nodes_norm.shape[1]

        outputs = torch.zeros(nodes_norm.shape[0], num_Ped, 3).cuda()
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum >= self.args.obs_length and iftest:
                node_index = self.get_node_index(seq_list[:self.args.obs_length])#Get node indices only from observation period
                updated_batch_pednum = self.update_batch_pednum(batch_pednum, node_index)
                st_ed = self.get_start_end(updated_batch_pednum)
                nodes_current = outputs[self.args.obs_length - 1:framenum, node_index]#get Predicted trajectory
            ...
            # Training phase
                ...
                node_index = self.get_node_index(seq_list[:framenum + 1])#Get node indices from all available frames
                nei_list = nei_lists[framenum, node_index, :]
                nei_list = nei_list[:, node_index]
                #nei_weight = nei_weights[framenum, node_index, :]
                #nei_weight = nei_weight[:, node_index]
                nei_weight = None
                updated_batch_pednum = self.update_batch_pednum(batch_pednum, node_index)
                st_ed = self.get_start_end(updated_batch_pednum)#compute the start and end indices for the agents in each scene 
            ...
            # Input Embedding for temporal transformer
            d = self.input_embedding_layer_temporal(nodes_current.to('cuda:0'))
            d= self.relu(d)
            temporal_input_embedded = self.dropout_in(d)

            # Input Embedding for spatial transformer
            spatial_input_embedded_ = self.dropout_in2(self.relu(self.input_embedding_layer_spatial(node_abs)))

            spatial_input_embedded = self.spatial_encoder_1(spatial_input_embedded_[-1].unsqueeze(1), nei_list, nei_weight)
            ...
            spatial_input_embedded = self.spatial_encoder_2(fusion_feat.unsqueeze(1), nei_list, nei_weight)
            ...
            #outputs_current = self.output_layer(temporal_input_embedded)
