...
class STAR(torch.nn.Module):
    def __init__(self, args, dropout_prob=0, nlayers = 2, nhead = 8, bilstm_hidden_size = 32):
        ...
        # Update LSTM layers with newer configurations
            ...
            # Use modern initialization for linear layers
            self.temporal_proj_1 = nn.Linear(bilstm_hidden_size * 2, 32)
            self.temporal_proj_2 = nn.Linear(bilstm_hidden_size * 2, 32)
        else:
    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            #Add graph memory (GM) to temporal transformer inpur for framenum > 0
            if framenum > 0:
            ...
            if self.args.useLSTM:
                # LSTM expects shape (seq_len, batch, input_size)
                lstm_out1, _ = self.temporal_encoder_1(temporal_input_embedded)
                #Take the last time step and project back to 32 dim
                temporal_input_embedded_last = self.temporal_proj_1(lstm_out1[-1])
            else:
                temporal_input_embedded_last = self.temporal_encoder_1(temporal_input_embedded)[-1]
            ...
            if self.args.useLSTM:
                lstm_out2, _ = self.temporal_encoder_2(temporal_input_embedded)
                temporal_input_embedded = self.temporal_proj_2(lstm_out2[-1])
            else:
